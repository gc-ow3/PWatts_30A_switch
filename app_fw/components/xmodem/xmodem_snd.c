...
#define xmMemAllocCtlStruct(n)	(xmCtrl_t *)calloc(1, sizeof(xmCtrl_t))
...
#define xmMemAllocRcvBuf(sz)		(uint8_t *)calloc(sz, 1)
...
static int xmSendBlock(xmCtrl_t * pCtrl, uint8_t * pData)
{
	...
	while (retries > 0) {
		...
		case XMCTL_CAN:
			ESP_LOGE(TAG, "Receiver cancelled transfer");
			retries = 0;
			break;

		default:
			// Unexpected reply - force loop to exit
			ESP_LOGE(TAG, "Unexpected response %08X", rdChar);
			//retries = 0;
			break;
		}
	}
